    ...
    if (st[1][0] == '#') {
        ...
        for (; chanItr != theChannel->userList_end(); ++chanItr) {
            ...
            bot->Reply(theClient, "[%s] (%s) %0.3lf %s (%s)",
                       bot->isNormal(targetClient) ? "N" : "A", targetClient->getCharYY().c_str(),
                       userEntropy, targetClient->getRealNickUserHost().c_str(),
        ...
        bot->Reply(theClient, "Range above/below/total     : %0.3lf / %0.3lf / %0.3lf",
                   maxEntropy - averageEntropy, averageEntropy - minEntropy,
