...
sqlSpamRule::sqlSpamRule(dbHandle* _SQLDb)
    : id(0), name(), description(), threshold(10), wait_on_rule_id(0), allchans(true),
      points_per("CLIENT"), score_globally(false), enabled(true), silent(false),
      report_source("BOT"), created_ts(0), modified_ts(0), modified_by(0), SQLDb(_SQLDb) {}
...
bool sqlSpamRule::insert() {
    ...
    q << "INSERT INTO spam_rules "
      ...
      << "'" << escapeSQLChars(name) << "', "
      << "'" << escapeSQLChars(description) << "', " << threshold << ", "
      << (wait_on_rule_id > 0 ? std::to_string(wait_on_rule_id) : "NULL") << ", "
      << (allchans ? "true" : "false") << ", "
      << "'" << escapeSQLChars(points_per) << "', " << (score_globally ? "true" : "false") << ", "
      << (enabled ? "true" : "false") << ", " << (silent ? "true" : "false") << ", "
      << "'" << escapeSQLChars(report_source) << "', " << created_ts << ", " << modified_ts << ", "
