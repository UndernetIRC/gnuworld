...
sqlSpyClient::sqlSpyClient(dbHandle* _SQLDb)
    : id(0), nickname(), username(), hostname(), ip(), realname(), account(), account_id(0),
      modes("+i"), enabled(true), created_by(0), created_ts(0), modified_ts(0), modified_by(0),
      SQLDb(_SQLDb) {}
...
bool sqlSpyClient::insert() {
    ...
    q << "INSERT INTO spyclients "
      ...
      << (account.empty() ? "NULL" : "'" + escapeSQLChars(account) + "'") << ", " << account_id
      << ", "
      << "'" << escapeSQLChars(modes) << "', " << (enabled ? "true" : "false") << ", "
      << (created_by > 0 ? std::to_string(created_by) : "NULL") << ", " << created_ts << ", "
      << modified_ts << ", " << (modified_by > 0 ? std::to_string(modified_by) : "NULL")
      << ") RETURNING id";
