...
sqlSpamRuleAction::sqlSpamRuleAction(dbHandle* _SQLDb)
    : id(0), rule_id(0), action_id(0), action_type(), action_duration_override(-1),
      action_reason_override(), delay_override(-1), SQLDb(_SQLDb), actionPtr(nullptr) {}
...
bool sqlSpamRuleAction::insert() {
    ...
    q << "INSERT INTO spam_rule_actions "
      ...
      << "action_duration_override, action_reason_override, delay_override) VALUES (" << rule_id
      << ", " << action_id << ", "
      << "'" << escapeSQLChars(action_type) << "', "
      << (action_duration_override >= 0 ? std::to_string(action_duration_override) : "NULL") << ", "
      << (action_reason_override.empty() ? "NULL"
                                         : ("'" + escapeSQLChars(action_reason_override) + "'"))
