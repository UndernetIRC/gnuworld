...
sqlSpamAction::sqlSpamAction(dbHandle* _SQLDb)
    : id(0), name(), action_type(), duration(3600), reason(), delay(0), rand_min(-1), rand_max(-1),
      enabled(true), prefix_auto(true), created_ts(0), modified_ts(0), modified_by(0),
      SQLDb(_SQLDb) {}
...
bool sqlSpamAction::insert() {
    ...
    q << "INSERT INTO spam_actions "
      ...
      << (reason.empty() ? "NULL" : ("'" + escapeSQLChars(reason) + "'")) << ", " << delay << ", "
      << (rand_min >= 0 ? std::to_string(rand_min) : "NULL") << ", "
      << (rand_max >= 0 ? std::to_string(rand_max) : "NULL") << ", " << (enabled ? "true" : "false")
      << ", " << (prefix_auto ? "true" : "false") << ", " << created_ts << ", " << modified_ts
