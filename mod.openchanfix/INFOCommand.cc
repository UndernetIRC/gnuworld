...
void INFOCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
    ...
    if (!theChan->useSQL()) {
        bot->logAdminMessage(
            "%s (%s) INFO %s", theUser ? theUser->getUserName().c_str() : "!NOT-LOGGED-IN!",
    ...
    if (noteCount > 0) {
        bot->SendTo(
            theClient,
            bot->getResponse(theUser, language::info_notes_count, std::string("Notes (%d):"))
                .c_str(),
            noteCount);
        ...
        for (unsigned int i = 0; i < noteCount; i++) {
            ...
            bot->SendTo(theClient,
                        bot->getResponse(theUser, language::info_notes,
                                         std::string("[%d:%s] %s \002%s\002%s%s"))
                            .c_str(),
                        note_id, from.c_str(), prettyTime(when).c_str(),
                        bot->getEventName(event).c_str(), (!theMessage.empty()) ? " " : "",
                        theMessage.c_str());
        }
    }
