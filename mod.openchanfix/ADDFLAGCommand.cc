...
void ADDFLAGCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
    ...
    if (flag == bot->getFlagChar(sqlcfUser::F_OWNER) && !theUser->getFlag(sqlcfUser::F_OWNER)) {
        bot->SendTo(theClient,
                    bot->getResponse(theUser, language::owner_add_owner_only,
    ...
    if (theUser->getFlag(sqlcfUser::F_SERVERADMIN) && !theUser->getFlag(sqlcfUser::F_USERMANAGER)) {
        if (targetUser->getGroup() != theUser->getGroup()) {
            bot->SendTo(theClient,
                        bot->getResponse(
                               theUser, language::cant_add_flag_diff_group,
        ...
        if (flag == bot->getFlagChar(sqlcfUser::F_SERVERADMIN)) {
            bot->SendTo(theClient,
                        bot->getResponse(theUser, language::cant_add_serveradmin_flag,
    ...
    bot->logAdminMessage(
        ...
        theClient->getRealNickUserHost().c_str(), targetUser->getUserName().c_str(), flag);
