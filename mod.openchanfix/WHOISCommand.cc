...
void WHOISCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
    ...
    if (st[1] == "*") {
        ...
        bot->SendTo(theClient,
                    bot->getResponse(theUser, language::number_of_users,
    ...
    if (theUser2->getIsSuspended())
        bot->SendTo(theClient, bot->getResponse(theUser, language::whois_suspended,
                                                std::string("--SUSPENDED--"))
                                   .c_str());

    if (!theUser2->getFlags())
        bot->SendTo(theClient, bot->getResponse(theUser, language::whois_flags_none,
    ...
    if (theUser2->getNeedOper())
        ...
        bot->SendTo(theClient, bot->getResponse(theUser, language::whois_needoper_no,
    ...
    if (st.size() > 2 && string_upper(st[2]) == "-MODIF")
        bot->SendTo(theClient,
                    bot->getResponse(theUser, language::whois_modif,
