...
bool sqlChannel::Delete(dbHandle* cacheCon) {
    ...
    if (!cacheCon->Exec(deleteString.str())) {
        elog << "sqlChannel::Delete> Something went wrong: " << cacheCon->ErrorMessage()
             << std::endl;
        retval = false;
    } else
        retval = true;
...
bool sqlChannel::commit(dbHandle* cacheCon) {
    ...
    if (!cacheCon->Exec(chanCommit.str())) {
        elog << "sqlChannel::commit> Something went wrong: " << cacheCon->ErrorMessage()
             << std::endl;
        retval = false;
    } else
        retval = true;
...
void sqlChannel::addNote(dbHandle* cacheCon, unsigned short eventType, iClient* theUser,
    ...
    if (!cacheCon->Exec(theLog.str())) {
        elog << "sqlChannel::addNote> Something went wrong: " << cacheCon->ErrorMessage()
             << std::endl;
    }
