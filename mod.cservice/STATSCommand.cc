    ...
    if (st.size() >= 2) {
        ...
        if (st[1][0] == '#') {
            ...
            if (ptr != bot->pendingChannelList.end()) {
                ...
                bot->Notice(theClient, "---- Pending channel stats for %s ----",
                            ptr->first.c_str());
                ...
                sqlPendingChannel::supporterListType::iterator sPtr =
                    tmpPending->supporterList.begin();

                for (; sPtr != tmpPending->supporterList.end(); ++sPtr) {
                    bot->Notice(theClient, " - User ID: %i, Join count: %i", sPtr->first,
                                sPtr->second);
                }
            ...
            if (theServer) {
                ...
                bot->Notice(theClient, " - Still Bursting? %s",
