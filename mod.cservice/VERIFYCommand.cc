...
bool VERIFYCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (!theUser) {
        if (target->isOper()) {
            bot->Notice(
                theClient,
                bot->getResponse(tmpUser, language::is_an_ircop, string("%s is an IRC operator"))
                    .c_str(),
                target->getNickUserHost().c_str());
        } else {
            bot->Notice(theClient,
                        bot->getResponse(tmpUser, language::is_not_logged_in,
    ...
    if (level >= level::admin::base) {
        if (theUser->getFlag(sqlUser::F_ALUMNI)) {
            bot->Notice(
                theClient,
                bot->getResponse(
                       tmpUser, language::is_cservice_alumni,
    ...
    if ((level >= level::admin::base) && (level <= level::admin::helper)) {
        bot->Notice(
            theClient,
            bot->getResponse(
                   tmpUser, language::is_cservice_rep,
    ...
    if ((level > level::admin::helper) && (level <= level::admin::admin)) {
        bot->Notice(theClient,
                    bot->getResponse(
                           tmpUser, language::is_cservice_admin,
                           string("%s is an Official CService Administrator%s and logged in as %s"))
                        .c_str(),
                    target->getNickUserHost().c_str(), extra.c_str(),
    ...
    %s")).c_str(), target->getNickUserHost().c_str(), extra.c_str(),
                    theUser->getUserName().c_str());
            return true;
            }
