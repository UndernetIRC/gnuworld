...
{ 
	...
	if(inChan < 1)
	{
		bot->Notice(theClient, "%s: You are not in that channel.", st[1].c_str());
		return true;
	}
	...
	if(i < 1) bot->Notice(theClient, "%s: ban list is empty.", st[1].c_str());
	else bot->Notice(theClient, "%s: End of ban list", st[1].c_str());
