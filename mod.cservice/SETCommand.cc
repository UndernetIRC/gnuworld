...
if (st.size() < 4)
	...
	if (option == "FLOODNET")
	{
		...
		bot->Notice(theClient,
			bot->getResponse(theUser,
					language::set_cmd_status,
					string("%s punishment for %s is %s")).c_str(),
					option.c_str(),
					theChan->getName().c_str(),
					value.c_str());
		if (theChan->getFloodNet() > prevFN)
		{
			bot->NoticeChannelOps(tmpChan,"Increased %s punishment for %s to %s by %s",
					option.c_str(),
					theChan->getName().c_str(),
					value.c_str(),
		...
		{
			bot->NoticeChannelOps(tmpChan,"Decreased %s punishment for %s to %s by %s",
					option.c_str(),
					theChan->getName().c_str(),
					value.c_str(),
					theUser->getUserName().c_str());
