...
bool MODINFOCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (command == "INVITE") { // Handle modinfo INVITE
        if (bot->getAccessLevel(theUser, theChan) == 0) {
            /* forced users cant set their invite status on, as they have no level record for the
             * channel */
            bot->Notice(
                theClient,
                bot->getResponse(
    ...
    if (command == "ACCESS") {
        ...
        if (level <= newAccess) {
            bot->Notice(
                theClient,
                bot->getResponse(theUser, language::cant_give_higher,
        ...
        if (level < 500) {
            string theMessage = TokenStringsParams(
                "%s modified %s's access on channel %s from %i to %i",
                theUser->getUserName().c_str(), targetUser->getUserName().c_str(),
                theChan->getName().c_str(), targetLevel, newAccess);
    ...
    if (command == "AUTOMODE") {
        ...
        if (st[1] == "*") { // Admin channel?
            bot->Notice(theClient, "If there's anything more important than my ego around, I want "
        ...
        if ((level < targetLevel) || ((level == targetLevel) && (targetUser != theUser))) {
            bot->Notice(
                theClient,
                bot->getResponse(theUser, language::mod_access_higher,
