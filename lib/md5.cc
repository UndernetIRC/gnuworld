...
*/
 ...
 MD5Transform ((long unsigned int*)context->state, context->buffer);

 for (i = partLen; i + 63 < inputLen; i += 64)
   MD5Transform ((long unsigned int*)context->state, &input[i]);
...
void MD5Final(unsigned char digest[16], MD5_CTX * context)  
{
  ...
  /* Save number of bits */
  Encode (bits, (long unsigned int*)context->count, 8);
...
*/
  ...
  Encode (digest, (long unsigned int*)context->state, 16);
