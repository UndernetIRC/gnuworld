...
ALTER TABLE channels ADD COLUMN limit_joinmax INT4 DEFAULT '5';
ALTER TABLE channels ADD COLUMN limit_joinsecs INT4 DEFAULT '60';
ALTER TABLE channels ADD COLUMN limit_joinperiod INT4 DEFAULT '180';
ALTER TABLE channels ADD COLUMN limit_joinmode VARCHAR(255) DEFAULT '+b *!~*@*';

UPDATE channels SET limit_joinmax = 5;
UPDATE channels SET limit_joinsecs = 60;
UPDATE channels SET limit_joinperiod = 180;
UPDATE channels SET limit_joinmode = '+b *!~*@*';
...
INSERT INTO help VALUES ('SET JOINPERIOD', 1, E'/msg X set <#channel> JOINPERIOD <1-600>\nThe period after JOINLIM has been triggered (and JOINMODE set) that X will remove the set JOINMODE.\nThe duration can be between 1-600 seconds. For each uidented and unauthenticated client joining while the JOINMODE is set, the JOINPERIOD will be extended with SECS second (see SET JOINMAX)');
