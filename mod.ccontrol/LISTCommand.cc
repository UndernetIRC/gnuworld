...
bool LISTCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (!strcasecmp(st[1].c_str(), "glines")) {
        ...
        bot->Notice(theClient, "Please use LIST nomodechannels to get a list of the channels which "
                               "are marked as no mode");
        ...
        if (!strcasecmp(st[2].c_str(), "key")) {
            if (st.size() < 4) {
                bot->Notice(theClient,
                            "for 'list channels key', you must supply the key to search for!");
                return false;
            }
            ...
            if (channelList.empty()) {
                bot->Notice(theClient, "I'm sorry, no channels have a key matching '%s'",
                            st[3].c_str());
                return false;
            }
            ...
            if (st.size() < 4) {
                ...
                bot->Notice(theClient,
                            "(other modes may be set unless explicitly not required as above)");
                return false;
            }
            if (st.size() > 4) {
                bot->Notice(theClient, "You supplied too many arguments.  arguments such as +k or "
                                       "+l here may not contain the key or limit!");
                return false;
            }
            ...
            if (channelList.empty()) {
                bot->Notice(theClient, "I'm sorry, no channels have modes matching '%s'",
                            st[3].c_str());
                return false;
            }
            ...
            if (channelList.empty()) {
                bot->Notice(theClient, "I'm sorry, no channels have a topic matching '%s'",
                            st[3].c_str());
                return false;
            }
            ...
            if (st.size() < 4) {
                bot->Notice(theClient, "for 'list channels topicby', you must supply the "
                                       "nick!user@host mask to search for!");
                return false;
            }
            ...
            if (string::npos == atPos) {
                ...
                Mask += "!*@*";
                bot->Notice(theClient,
                            "Mask was not in nick!user@host format, re-writing mask to '%s'.  This "
                ...
                if ((string::npos == atPos2) || (atPos2 < atPos)) {
                    ...
                    Mask += "@*";
                    bot->Notice(theClient,
                                "Mask was not in nick!user@host format, re-writing mask to '%s'.  "
