        ...
        if (!strcasecmp(st[pos], "-CClonesTime")) {
            ...
            if (!IsTimeSpec(st[pos + 1])) {
                bot->Notice(theClient,
                            "-CClonesTime must have a valid time specified, e.g. 60s or 1m");
                return true;
            }
            ...
            if (st.size() < pos + 2) {
                bot->Notice(theClient,
                            "-CClonesGTime must get the duration of the gline in seconds.");
                return true;
            }
            if (!IsTimeSpec(st[pos + 1])) {
            ...
            int g = extractTime(st[pos + 1], 1);
            if (g < 1800 || g > 172800) {
                bot->Notice(theClient, "-CClonesGTime value must be between 1800 and 172800 "
                                       "seconds (30 mins - 2 days).");
                return true;
            }
            ...
            if (st.size() < pos + 2) {
                bot->Notice(theClient,
                            "-GBInterval must get the number of seconds between each burst");
                return true;
            }
            if (!IsTimeSpec(st[pos + 1])) {
                bot->Notice(
            ...
            if (st.size() < pos + 2) {
                bot->Notice(theClient, "-GTime must get the duration for the excessive connections "
                                       "gline - e.g. 900s, 30m, 1h or 1d.");
                return true;
            }
            if (!IsTimeSpec(st[pos + 1])) {
                bot->Notice(theClient,
                            "-GTime must get a valid duration - e.g. 900s, 30m, 1h or 1d.");
                return true;
            }
            int g = extractTime(st[pos + 1], 1);
            if (g < 300 || g > 172800) {
