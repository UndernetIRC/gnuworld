    ...
    while (pos < st.size()) {
        ...
        } else if (!strcasecmp(st[pos], "-gl")) // Trying to toggle the get of logs
        {
            ...
            if (!strcasecmp(st[pos + 1], "on")) {
                ...
                bot->Notice(theClient, "unknown option %s for -gl must be on/off",
                            st[pos + 1].c_str());
                return false;
            }
            tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
        ...
        else if (!strcasecmp(st[pos], "-sso")) // Trying to toggle the Single Sign On
        {
            ...
            if (!strcasecmp(st[pos + 1], "on")) {
                ...
                bot->Notice(theClient, "unknown option %s for -sso must be on/off",
                            st[pos + 1].c_str());
                return false;
            }
            tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
        ...
        else if (!strcasecmp(st[pos],
                             "-ssooo")) // Trying to toggle the Single Sign On if Opered Only
        {
            if (AdFlag < operLevel::SMTLEVEL) {
                bot->Notice(theClient,
                            "Single sign on with oper requirement can only be modified by SMT+");
                return false;
            }
            if ((pos + 1) >= st.size()) {
            ...
            if (!strcasecmp(st[pos + 1], "on")) {
                ...
                bot->Notice(theClient,
                            "Single sign on with oper requirement has been turned on for %s",
                            st[1].c_str());
            } else if (!strcasecmp(st[pos + 1], "off")) {
                ...
                bot->Notice(theClient,
                            "Single sign on with oper requirement has been turned off for %s",
                            st[1].c_str());
            } else {
                bot->Notice(theClient, "unknown option %s for -ssooo must be on/off",
                            st[pos + 1].c_str());
                return false;
            }
            tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
        ...
        else if (!strcasecmp(st[pos], "-autoop")) // Trying to toggle autoop
        {
            ...
            if (!strcasecmp(st[pos + 1], "on")) {
                ...
                bot->Notice(theClient, "unknown option %s. -autoop must be on/off",
                            st[pos + 1].c_str());
            }
            tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
            tmpUser->Update();
        ...
        else if (!strcasecmp(st[pos], "-glag")) // Trying to toggle the get of lag reports
        {
            ...
            if (!strcasecmp(st[pos + 1], "on")) {
                ...
                bot->Notice(theClient, "unknown option %s for -glag must be on/off",
                            st[pos + 1].c_str());
                return false;
            }
            tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
        ...
        else if (!strcasecmp(st[pos], "-s")) // Trying to change the user server
        {
            ...
            if (!strcasecmp(tmpUser->getServer(), SName)) {
                bot->Notice(theClient, "%s already is associated with %s", SName.c_str(),
                            SName.c_str());
                pos += 2;
            } else {
                ...
                tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
                if (tmpUser->Update()) {
                    bot->Notice(theClient, "%s has been associated with %s", st[1].c_str(),
                                SName.c_str());
                } else {
        ...
        } else if (!strcasecmp(st[pos], "-op")) // Trying to toggle needop
        {
            ...
            if (!strcasecmp(st[pos + 1], "on")) {
                ...
                bot->Notice(theClient, "unknown option %s for -no must be on/off",
                            st[pos + 1].c_str());
            }
            tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
            tmpUser->Update();
        ...
        } else if (!strcasecmp(st[pos], "-ua")) // Trying to update the access?
        {
            if ((AdFlag < operLevel::CODERLEVEL) && (AdFlag <= OpFlag)) {
                ...
                tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
                if (tmpUser->Update()) {
                    bot->Notice(theClient, "Successfully updated access level for %s.",
                                st[1].c_str());
                } else {
                    bot->Notice(theClient, "Error while updating access level for %s.",
                                st[1].c_str());
                }
            }
            pos++;
        ...
        } else if (!strcasecmp(st[pos], "-e")) // Trying to change email
        {
            ...
            tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
            if (tmpUser->Update()) {
                bot->Notice(theClient, "Successfully updated e-mail address for %s.",
                            st[1].c_str());
            } else {
                bot->Notice(theClient, "Error while updating e-mail address for %s.",
                            st[1].c_str());
            }
            pos += 2;
        } else {
