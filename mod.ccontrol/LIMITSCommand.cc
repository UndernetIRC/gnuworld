...
bool LIMITSCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (!strcasecmp(st[1].c_str(), "info")) {
        ...
        if (IpLisp != 0) {
            ...
            if (bot->insertIpLisp(theClient, st[2], atoi(st[3].c_str()), atoi(st[4].c_str()), email,
                                  1, 0)) {
        ...
        if (IpLisp == 0) {
            ...
            if (bot->insertIpLnb(theClient, st[3], IpLisp->getID(), false)) {
                bot->Notice(theClient, "Successfully added '%s' to '%s'", st[3].c_str(),
                            st[2].c_str());
            }
        }
        ...
        if (!IpLisp->updateData()) {
            ...
            bot->Notice(theClient, "Email for %s set to: %s", IpLisp->getName().c_str(),
                        st[3].c_str());
        ...
        if (IpLisp == 0) {
            ...
            if (!IpLisp->updateData()) {
                ...
                bot->Notice(theClient, "New Ident limit for %s is now %d",
                            IpLisp->getName().c_str(), IpLisp->getIdentLimit());
        }
        ...
        if (IpLisp == 0) {
            ...
            if (!IpLisp->updateData()) {
                ...
                bot->Notice(theClient, "Renamed %s to %s", st[2].c_str(),
                            IpLisp->getName().c_str());
        }
