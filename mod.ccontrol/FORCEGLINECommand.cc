...
bool FORCEGLINECommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (gCheck & gline::BAD_CIDRMASK) {
        bot->Notice(theClient,
                    "The IP listed is not on a bit boundary for the CIDR mask specified.");
        Ok = false;
    }
    ...
    if ((gCheck & gline::FU_NEEDED_USERS) && (Ok)) {
        if (!Forced) {
            ...
            if (tmpUser->getFlags() < operLevel::SMTLEVEL) {
                bot->Notice(theClient,
                            "Sorry, you can't set a gline which affects more than %d users",
                            gline::MFGLINE_USERS);
            } else {
                bot->Notice(theClient,
                            "This gline affects more than %d users, please use the -fu flag",
                            gline::MFGLINE_USERS);
            }
        }
    }
    if ((gCheck & gline::FU_NEEDED_TIME) && (Ok)) {
        if (!Forced) {
            ...
            if (tmpUser->getFlags() < operLevel::SMTLEVEL) {
                ...
                bot->Notice(theClient,
                            "This gline is for more than %d seconds, please use the -fu flag",
                            gline::MFGLINE_TIME);
            }
        }
    }
    if ((gCheck & gline::HUH_IS_IP_OF_OPER) && (Ok)) {
        if (bot->isGlinedException(userName + "@" + hostName) > 0) {
            ...
            bot->Notice(theClient,
                        "There is someone who previously opered from that host (%s). Send the "
                        "gline again to force.",
    ...
    if(gCheck & gline::FORCE_NEEDED_WILDTIME)
            {
            bot->MsgChanLog("%s is using forcegline to gline a wildcard host for more than %d
    seconds" ,theClient->getNickName().c_str() ,gline::MGLINE_WILD_TIME);
            }*/
